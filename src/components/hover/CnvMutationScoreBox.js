  ...
  render() {
    ...
    if(cohortIndex===0) {
      if (data.pathway.firstSampleCnvHigh > 0) {
        returnArray.push(
          <Chip><span className={BaseStyle.cnvHighColor}><strong>CNV Amplification</strong><ScoreBadge
            score={data.pathway.firstSampleCnvHigh}/></span></Chip>
        )
      }
      if (data.pathway.firstSampleCnvLow > 0) {
        returnArray.push(
          <Chip><span className={BaseStyle.cnvLowColor}><strong>CNV Deletion</strong><ScoreBadge
            score={data.pathway.firstSampleCnvLow}/></span></Chip>
        )
      }
      if (data.pathway.firstSampleMutation2 > 0) {
        returnArray.push(
          <Chip><span className={BaseStyle.mutation2Color}><strong>Missense / Inframe</strong><ScoreBadge
            score={data.pathway.firstSampleMutation2}/></span></Chip>
        )
      }
      if (data.pathway.firstSampleMutation3 > 0) {
        returnArray.push(
          <Chip><span className={BaseStyle.mutation3Color}><strong>Splice</strong><ScoreBadge
            score={data.pathway.firstSampleMutation3}/></span></Chip>
        )
      }
      if (data.pathway.firstSampleMutation4 > 0) {
        returnArray.push(
          <Chip><span className={BaseStyle.mutation4Color}><strong>Deleterious</strong><ScoreBadge
